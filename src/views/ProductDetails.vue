...
<template>
    <Layout>
        ...
        <div v-if="product.image" class="container" id="mainProduct">
            <div class="card">
                <div class="container-fliud">
                    <div class="wrapper row">
                        <div class="preview col-md-6">
                            <div class="preview-pic tab-content">
                                <div class="tab-pane active" id="pic-1"><img :src="mainImage" /></div>
                            </div>
                            <ul class="preview-thumbnail nav nav-tabs">
                                <li @click="changeImage(product.image)">
                                    <a data-toggle="tab"><img :src="product.image" /></a>
                                </li>
                                <li v-for="(flav,index) in flavours" :key="index" @click="changeImage(flav.image, flav.name)">
                                    <a data-toggle="tab"><img :src="flav.image" /></a>
                                </li>
                            </ul>
                        </div>
                            ...
                            <table class="custom_table">
                                ...
                                <tbody>
                                    <tr v-for="(ds, index) in multiPayment" :key="index">
                                        <td scope="row">{{ds.min + (ds.max ? ' - ' + ds.max : ' +') }}</td>
                                        <td> £{{ds.discount}} </td>
                                    </tr>
                                </tbody>
                            </table>
                            <a-alert v-if="!getFlavourChoose" style="margin-bottom: 10px;" message="Can you please choose a flavour" type="error" />
                            <h5 class="sizes">Flavour:
                                <a-select
                                        v-model:value="flavour"
                                        style="width: 100%; marginTop: 10px;"
                                        @change="handleChange"
                                    >
                                        <a-select-option v-for="(flav,index) in flavours" :key="index" :value="flav.name">{{flav.name}}</a-select-option>
                                    </a-select>
                            </h5>
                            <div class="action">
                                <button class="add-to-cart btn btn-default" type="button"  @click="addItem">add to cart</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <a-tabs v-model:activeKey="activeKey">
                        <a-tab-pane key="1" tab="Flavours"><InfoFlavours :flavours="flavours"/></a-tab-pane>
                        <a-tab-pane key="2" tab="Additional information" force-render></a-tab-pane>
...
import SubscribePart from '../components/SubscribePart.vue'
